...
On a fresh deployment with no existing users, Feloxi shows a **setup wizard** at first visit. The wizard creates the first organization and admin account.

1. Open **http://localhost:3000** — you'll be redirected to the setup page
2. Fill in your organization name, slug, email, and password
3. Click **Complete setup** — you're logged in as the admin

After the first user is created, the setup wizard is disabled and all subsequent visits go to the login page.

By default, public registration is disabled (`ALLOW_SIGNUP=false`). New users can only be added via **Settings > Team > Invite**. To allow open registration, set `ALLOW_SIGNUP=true` in your environment.

## Verify Installation (Demo Mode)

The default `docker-compose.yml` includes a seed container that creates demo data. After seeding, you can sign in with:
...
| `ALLOW_SIGNUP`   | `false` (default) — set `true` for open signup   |
