...
pub async fn create_alert_rule(pool: &PgPool, input: &CreateAlertRule) -> Result<AlertRule, AppError> {
...
pub async fn list_enabled_alert_rules(pool: &PgPool, tenant_id: Uuid) -> Result<Vec<AlertRule>, AppError> {
    let rules = sqlx::query_as::<_, AlertRule>(
...
pub async fn count_alert_history(
    pool: &PgPool,
    tenant_id: Uuid,
) -> Result<i64, AppError> {
    let (count,): (i64,) =
