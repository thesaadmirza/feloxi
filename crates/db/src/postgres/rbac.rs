...
pub async fn init_system_roles(pool: &PgPool, tenant_id: Uuid) -> Result<(), AppError> {
    let system_roles: Vec<(&str, Vec<String>)> = vec![
        (
            "admin",
            vec![
                "tasks_read", "tasks_retry", "tasks_revoke",
                "workers_read", "workers_shutdown",
                "alerts_read", "alerts_write",
                "beat_read", "metrics_read",
                "settings_read", "settings_write",
                "team_manage", "api_keys_manage", "brokers_manage",
            ]
            .into_iter()
            .map(String::from)
            .collect(),
        ...
        (
            "editor",
            vec![
                "tasks_read", "tasks_retry",
                "workers_read",
                "alerts_read", "alerts_write",
                "beat_read", "metrics_read",
                "settings_read", "api_keys_manage", "brokers_manage",
            ]
            .into_iter()
            .map(String::from)
            .collect(),
        ...
        (
            "viewer",
            vec![
                "tasks_read", "workers_read",
                "alerts_read", "beat_read", "metrics_read",
            ]
            .into_iter()
            .map(String::from)
            .collect(),
