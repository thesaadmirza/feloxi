...
pub async fn set_json<T: Serialize>(
    ...
    pool.set::<(), _, _>(key, json.as_str(), Some(Expiration::EX(ttl.as_secs() as i64)), None, false).await?;
    Ok(())
}
...
pub async fn set_queue_depth(
    ...
    pool.set::<(), _, _>(&key, depth.to_string().as_str(), Some(Expiration::EX(120)), None, false).await?;
...
pub async fn get_online_workers(
    pool: &Pool,
    tenant_id: Uuid,
...
pub async fn set_alert_cooldown(
    ...
    let key = keys::alert_cooldown(tenant_id, rule_id);
    pool.set::<(), _, _>(&key, "1", Some(Expiration::EX(cooldown_secs as i64)), None, false).await?;
    Ok(())
}
