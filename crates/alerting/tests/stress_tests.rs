...
fn make_condition(index: usize) -> (AlertCondition, EvaluationContext) {
    match index % 7 {
        ...
        1 => (
            AlertCondition::QueueDepth {
                threshold: 100,
                queue: format!("queue-{}", index % 4),
            },
            EvaluationContext {
                queue_depth: 150 + (index % 1000) as u64,
                ..Default::default()
            },
        ),
        2 => (
        ...
        3 => (
            ...
            EvaluationContext {
                p95_runtime: 15.0 + (index % 30) as f64,
                ..Default::default()
            },
        ),
        4 => (
            AlertCondition::BeatMissed {
                schedule_name: format!("schedule-{}", index),
            },
            EvaluationContext {
        ...
        5 => (
            AlertCondition::TaskFailed {
                task_name: format!("tasks.payment_{}", index),
            },
            EvaluationContext {
                recent_failures: 1 + (index % 10) as u64,
                ..Default::default()
            },
        ),
        _ => (
...
fn stress_throttle_cleanup_under_load() {
    ...
    let all_throttled = rule_ids
        .iter()
        .all(|&id| throttle.is_throttled(id, Duration::from_secs(300)));
    ...
    let any_throttled = rule_ids
        .iter()
        .any(|&id| throttle.is_throttled(id, Duration::from_secs(300)));
    assert!(
        !any_throttled,
        "No rules should be throttled after cleanup"
    );
...
fn stress_template_rendering_throughput() {
    ...
    println!(
        "Avg plain size:    {} bytes",
        plain_total_len / num_renders
    );
    println!(
        "Avg HTML size:     {} bytes",
        html_total_len / num_renders
    );
...
fn stress_template_rendering_large_content() {
    ...
    println!("Time:          {:?}", elapsed);
    println!(
        "Throughput:    {:.0} render-pairs/sec",
        num_renders as f64 / elapsed.as_secs_f64()
    );
}
