...
mod tests {
    ...
    fn condition_queue_depth_serde() {
        let cond = AlertCondition::QueueDepth { threshold: 1000, queue: "default".into() };
        let json = serde_json::to_string(&cond).unwrap();
    ...
    fn condition_worker_offline_serde() {
        let cond = AlertCondition::WorkerOffline { grace_period_seconds: 120 };
        let json = serde_json::to_string(&cond).unwrap();
    ...
    fn condition_beat_missed_serde() {
        let cond = AlertCondition::BeatMissed { schedule_name: "cleanup".into() };
        let json = serde_json::to_string(&cond).unwrap();
    ...
    fn condition_task_failed_serde() {
        let cond = AlertCondition::TaskFailed { task_name: "tasks.payment".into() };
        let json = serde_json::to_string(&cond).unwrap();
    ...
    fn condition_no_events_serde() {
        let cond = AlertCondition::NoEvents { silence_minutes: 15 };
        let json = serde_json::to_string(&cond).unwrap();
