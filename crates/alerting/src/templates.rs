...
mod tests {
    ...
    fn make_alert(severity: &str, rule_name: &str, summary: &str) -> FiredAlert {
        FiredAlert {
            ...
            rule_name: rule_name.into(), severity: severity.into(),
            summary: summary.into(), details: serde_json::json!({}), fired_at: 1700000000.0,
        }
    }
    ...
    fn plain_text_format() {
        let cases = [
            ("critical", "Worker Alert", "2 workers offline", "[CRITICAL] Worker Alert: 2 workers offline"),
            ("warning", "Queue Alert", "Queue depth high", "[WARNING] Queue Alert: Queue depth high"),
    ...
    fn html_severity_colors_and_structure() {
        let cases = [("critical", "#dc2626"), ("warning", "#f59e0b"), ("info", "#10b981"), ("unknown", "#10b981")];
