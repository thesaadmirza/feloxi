...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        let body = ErrorResponse {
            error: ErrorBody { code: self.error_code(), message: self.user_message() },
        };
...
mod tests {
    ...
    fn display_and_status_codes_all_variants() {
        let cases: Vec<(AppError, &str, StatusCode, &str)> = vec![
            (
                AppError::NotFound("user 123".into()),
                "Not found: user 123",
            ...
            (
                AppError::Internal("panic".into()),
                "Internal error: panic",
                StatusCode::INTERNAL_SERVER_ERROR,
                "INTERNAL_ERROR",
            ),
            (
                AppError::Database("conn".into()),
                "Database error: conn",
                StatusCode::INTERNAL_SERVER_ERROR,
                "DATABASE_ERROR",
            ),
            (
                AppError::Validation("name".into()),
