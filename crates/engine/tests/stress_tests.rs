...
fn make_dag_node(id: usize, group: Option<usize>) -> DagNode {
    DagNode {
        ...
        .to_string(),
        runtime: if id.is_multiple_of(4) { Some(0.5 + (id % 100) as f64 * 0.01) } else { None },
        queue: format!("queue-{}", id % 4),
        worker_id: format!("celery@worker-{}", id % 8),
        group_id: group.map(|g| format!("group-{:04}", g)),
        parent_id: if id > 0 && id % 10 >= 3 { Some(format!("task-{:06}", id - 1)) } else { None },
        chord_id: None,
    }
}
...
fn make_raw_task_event(index: usize) -> RawTaskEvent {
    RawTaskEvent {
        ...
        result: if index % 5 == 3 { Some(format!("\"result_{}\"", index)) } else { None },
        ...
        runtime: if index % 5 >= 3 { Some(0.1 + (index % 100) as f64 * 0.01) } else { None },
        retries: Some((index % 3) as u32),
        eta: None,
        expires: None,
        root_id: Some(format!("root-{:04}", index / 10)),
        parent_id: if index.is_multiple_of(3) {
            Some(format!("task-{:06}", index.saturating_sub(1)))
        } else {
            None
        },
        group_id: if index.is_multiple_of(7) {
...
fn stress_dag_resolution_at_scale() {
    ...
    for &size in &dag_sizes {
        ...
        let has_chain =
            dag.edges.iter().any(|e| matches!(e.edge_type, engine::dag_resolver::EdgeType::Chain));
        let has_group =
            dag.edges.iter().any(|e| matches!(e.edge_type, engine::dag_resolver::EdgeType::Group));
...
fn stress_dag_resolution_50k_tasks() {
    ...
    for i in 0..size {
        ...
        let group = if i % 10 == 0 { Some(i / 30) } else { None };
        tasks.push(make_dag_node(i, group));
    }
    ...
    assert_eq!(dag.nodes.len(), size);
    assert!(elapsed.as_secs() < 60, "50k DAG took too long: {:?}", elapsed);
}
...
fn stress_event_normalization_throughput() {
    ...
    let raw_events: Vec<RawTaskEvent> = (0..num_events).map(make_raw_task_event).collect();
