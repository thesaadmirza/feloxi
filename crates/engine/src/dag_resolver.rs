...
pub fn build_dag(tasks: Vec<DagNode>) -> WorkflowDag {
    ...
    let mut unconnected: Vec<&DagNode> = tasks
        .iter()
        .filter(|t| !connected.contains(&t.task_id) && t.group_id.is_none())
        .collect();
...
mod tests {
    ...
    fn build_dag_chain_two_tasks() {
        let tasks = vec![
            make_node("a", "task_a", None),
            make_node("b", "task_b", None),
        ];
    ...
    fn build_dag_chain_three_tasks() {
        ...
        let chain_edges: Vec<_> = dag
            .edges
            .iter()
            .filter(|e| matches!(e.edge_type, EdgeType::Chain))
            .collect();
    ...
    fn build_dag_group_three_members() {
        ...
        let group_edges: Vec<_> = dag
            .edges
            .iter()
            .filter(|e| matches!(e.edge_type, EdgeType::Group))
            .collect();
    ...
    fn build_dag_root_is_first_task() {
        let tasks = vec![
            make_node("z", "last", None),
            make_node("a", "first", None),
        ];
        let dag = build_dag(tasks);
