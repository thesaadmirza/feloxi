...
use crate::routes::responses::{
    BrokerListResponse, QueueListResponse, StringListResponse,
...
pub async fn stop_broker(
    ...
    db::postgres::broker_configs::set_broker_active(&state.pg, id, user.tenant_id, false)
        .await?;
    state.broker_manager.stop(id);

    db::postgres::broker_configs::update_broker_config_status(
        &state.pg,
        id,
        "disconnected",
        None,
