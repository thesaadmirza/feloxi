...
fn auth_cookies(access_token: &str, refresh_token: &str) -> HeaderMap {
    ...
    headers.append(
        ...
        access_cookie
            .parse()
            .expect("access token cookie is a valid HeaderValue"),
    );
    ...
    headers.append(
        ...
        refresh_cookie
            .parse()
            .expect("refresh token cookie is a valid HeaderValue"),
    );
...
pub async fn register(
    ...
    // Assign admin role
    let admin_role =
        db::postgres::rbac::get_role_by_name(&state.pg, tenant.id, "admin").await?;
...
pub async fn login(
    ...
    // Resolve tenant + user.
    // If tenant_slug is provided, look up directly. Otherwise, resolve from email.
    let resolved = if let Some(slug) = &req.tenant_slug {
        if slug.is_empty() {
            ...
            let u = db::postgres::users::get_user_by_email(&state.pg, t.id, &req.email)
                .await
                .map_err(|_| AppError::Unauthorized("Invalid credentials".into()))?;
            ResolveResult::Single(t, u)
        }
    } else {
        resolve_user_by_email(&state, &req.email).await?
    };

    // If multiple orgs, return the org picker response (no auth yet)
    let (tenant, user) = match resolved {
        ResolveResult::Single(t, u) => (t, u),
        ResolveResult::Multiple(orgs) => {
            let body = OrgPickerResponse {
...
enum ResolveResult {
    Single(db::postgres::models::Tenant, db::postgres::models::User),
    ...
    match users.len() {
        ...
        1 => {
            ...
            let tenant = db::postgres::tenants::get_tenant_by_id(&state.pg, user.tenant_id)
                .await
                .map_err(|_| AppError::Unauthorized("Invalid credentials".into()))?;
            Ok(ResolveResult::Single(tenant, user))
        }
        _ => {
            // Multiple orgs — gather the list for the user to pick
