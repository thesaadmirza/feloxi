...
pub async fn list_tasks(
    ...
    let next_cursor = if has_more {
        events.last().map(|e| {
            let ms = e.timestamp.unix_timestamp() * 1000
                + e.timestamp.millisecond() as i64;
            ms.to_string()
        })
    } else {
        None
...
pub async fn get_task(
    ...
    let latest = db::clickhouse::task_events::get_task_latest(
        &state.ch,
        user.tenant_id,
        &task_id,
    )
...
pub async fn get_task_timeline(
    ...
    let timeline = db::clickhouse::task_events::get_task_timeline(
        &state.ch,
        user.tenant_id,
        &task_id,
    )
