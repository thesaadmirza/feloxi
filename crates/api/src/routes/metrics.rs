...
use crate::routes::responses::{OverviewResponse, ThroughputResponse, QueueMetricsResponse, StringListResponse};
...
pub async fn overview(
    ...
    let stats = db::clickhouse::aggregations::get_overview_stats(
        &state.ch,
        user.tenant_id,
        from,
    )
...
pub async fn queue_metrics(
    ...
    let metrics = db::clickhouse::aggregations::get_queue_metrics(
        &state.ch,
        user.tenant_id,
        queue,
        from,
