...
pub async fn dashboard_ws(
    ...
    let token = params.token.or_else(|| extract_cookie_token(&headers));
    ...
    ws.on_upgrade(move |socket| handle_dashboard_socket(socket, state, claims.tid)).into_response()
}

async fn handle_dashboard_socket(socket: WebSocket, state: AppState, tenant_id: uuid::Uuid) {
...
fn matches_any_subscription(
    ...
    for topic in subscriptions {
        let matches = match (&event.payload, topic) {
            ...
            (
                EventPayload::TaskUpdate { task_id, .. },
                SubscriptionTopic::Task { task_id: tid },
            ) => task_id == tid,
            (
                EventPayload::TaskUpdate { task_name, .. },
                SubscriptionTopic::TaskType { task_name: tn },
            ) => task_name == tn,
            (EventPayload::WorkerUpdate { .. }, SubscriptionTopic::AllWorkers) => true,
            (
                EventPayload::WorkerUpdate { worker_id, .. },
                SubscriptionTopic::Worker { worker_id: wid },
            ) => worker_id == wid,
...
mod tests {
    ...
    fn make_beat_event() -> TenantEvent {
        TenantEvent {
            tenant_id: Uuid::nil(),
            payload: EventPayload::BeatUpdate {
                schedule_name: "cleanup".into(),
                task_name: "tasks.cleanup".into(),
                last_run_at: Some(1700000000.0),
                next_run_at: Some(1700003600.0),
            },
        }
    }
    fn make_alert_event() -> TenantEvent {
        TenantEvent {
            tenant_id: Uuid::nil(),
            payload: EventPayload::AlertFired {
                rule_id: Uuid::nil(),
                rule_name: "High failure rate".into(),
                severity: "critical".into(),
                summary: "Failure rate > 50%".into(),
            },
        }
    }
    fn make_metrics_event() -> TenantEvent {
        TenantEvent {
            tenant_id: Uuid::nil(),
            payload: EventPayload::MetricsSummary {
                throughput: 100.0,
                failure_rate: 0.05,
                active_workers: 3,
                queue_depth: 10,
    ...
    fn single_topic_matching() {
        ...
        type Case = (HashSet<SubscriptionTopic>, Vec<(TenantEvent, bool)>);
        let cases: Vec<Case> = vec![
            ...
            (
                [SubscriptionTopic::TaskType { task_name: "tasks.send_email".into() }]
                    .into_iter()
                    .collect(),
                vec![
            ...
            (
                [SubscriptionTopic::Worker { worker_id: "celery@host1".into() }]
                    .into_iter()
                    .collect(),
                vec![
                    (make_worker_event("celery@host1"), true),
        ...
        for (i, (subs, events)) in cases.iter().enumerate() {
            for (j, (event, expected)) in events.iter().enumerate() {
                assert_eq!(
                    matches_any_subscription(event, subs),
                    *expected,
    ...
    fn multiple_subscriptions_match_if_any_matches() {
        ...
        assert!(matches_any_subscription(
            &make_task_event("t1", "tasks.send_email", "emails", "w1"),
            &subs
        ));
        assert!(!matches_any_subscription(
            &make_task_event("t2", "tasks.add", "default", "w1"),
            &subs
        ));
        assert!(matches_any_subscription(&make_worker_event("w1"), &subs));
        assert!(matches_any_subscription(&make_alert_event(), &subs));
    ...
    fn default_subscriptions_match_expected_events() {
        let subs: HashSet<_> = [
            SubscriptionTopic::AllTasks,
            SubscriptionTopic::AllWorkers,
            SubscriptionTopic::Alerts,
            SubscriptionTopic::MetricsSummary,
        ]
        .into_iter()
        .collect();
