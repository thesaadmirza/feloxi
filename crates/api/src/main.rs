...
async fn main() -> Result<()> {
    ...
    tracing::info!("ClickHouse client configured");
...
async fn run_alert_evaluation(
    ...
    use alerting::{
        engine::{evaluate_condition, determine_severity, generate_summary, FiredAlert},
        rules::{AlertChannel, ResolvedAlertRule},
    };
    use db::postgres::models::ChannelDeliveryStatus;

    // Get all tenants
    let tenants = db::postgres::tenants::list_tenants(&state.pg, 1000, 0)
        .await
    ...
    for tenant in &tenants {
        ...
        for db_rule in &db_rules {
            ...
            details.insert("recent_failures".to_string(), serde_json::Value::from(ctx.recent_failures));
...
async fn build_evaluation_context(
    ...
    if let Ok(stats) = db::clickhouse::aggregations::get_overview_stats(
        &state.ch, tenant_id, 5,
    ).await {
        if stats.total_tasks > 0 {
            ctx.failure_rate = stats.failure_count as f64 / stats.total_tasks as f64;
        }
        ctx.p95_runtime = stats.p95_runtime;
        ctx.recent_failures = stats.failure_count as u64;
    }
...
async fn enforce_retention(state: &AppState) -> Result<(), String> {
    ...
    for tenant in &tenants {
        let policies =
            db::postgres::retention::list_retention_policies(&state.pg, tenant.id)
                .await
                .unwrap_or_default();
        for p in &policies {
    ...
    db::clickhouse::retention::apply_table_ttl(&state.ch, "worker_events", min_worker_days)
        .await?;
