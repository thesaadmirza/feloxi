...
pub async fn redis_revoke_task(
    ...
    let signal = if terminate { "SIGTERM" } else { "SIGTERM" };
...
pub async fn redis_shutdown_worker(
    connection_url: &str,
    hostname: &str,
) -> Result<(), String> {
    redis_broadcast_control(
...
pub async fn amqp_publish_task(
    ...
    let properties = lapin::BasicProperties::default()
        ...
        .with_headers({
            ...
            headers.insert(
                "task".into(),
                lapin::types::AMQPValue::LongString(task_name.into()),
            );
            headers.insert(
                "id".into(),
                lapin::types::AMQPValue::LongString(task_id.into()),
            );
            headers.insert(
                "root_id".into(),
                lapin::types::AMQPValue::LongString(task_id.into()),
            ...
            headers.insert(
                "lang".into(),
                lapin::types::AMQPValue::LongString("py".into()),
            );
            headers.insert(
                "origin".into(),
                lapin::types::AMQPValue::LongString("feloxi".into()),
            );
            headers
        });
...
pub async fn amqp_shutdown_worker(
    connection_url: &str,
    hostname: &str,
) -> Result<(), String> {
    amqp_broadcast_control(
